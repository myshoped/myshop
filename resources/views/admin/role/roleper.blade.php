...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <form role="form" action="/admin/doroleper?rid={{$res->id}}" method="post" class="form-horizontal">
            {{csrf_field()}}
            @if (count($errors) > 0)
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <input type="text" name="rolename" class="form-control" id="field-1" value="{{$res->rolename}}" readonly>
                </div>
            </div>
                ...
                <div class="col-sm-10">
                    ...
                    @foreach ($pers as $k=>$v)
                        @if(in_array($v->id, $arr))
                            <label>
                                <input type="checkbox" name="perid[]" value="{{$v->id}}" checked>
                        ...
                        @else
                            <label>
                                <input type="checkbox" name="perid[]" value="{{$v->id}}">
                                <div style='display: inline;'>
                                    <ul>{{$v->pername}}</ul>
                                </div>
                            </label>
